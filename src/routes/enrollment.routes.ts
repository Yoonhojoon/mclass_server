...
import { EnrollmentRepository } from '../domains/enrollment/enrollment.repository.js';
...
export const createEnrollmentRoutes = (prisma: PrismaClient): Router => {
  const router = Router();
  const repository = new EnrollmentRepository(prisma);
  const mclassRepository = new MClassRepository(prisma);
  const enrollmentFormService = new EnrollmentFormService(
